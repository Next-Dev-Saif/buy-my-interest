    ...
    <section className="py-32 relative overflow-hidden">
      ...
      <Container>
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          ...
          <Link href="/get-started" className="group flex items-center gap-3 px-8 py-4 rounded-2xl bg-primary text-primary-foreground font-bold text-sm hover:bg-primary/90 hover:shadow-xl hover:shadow-primary/20 transition-all active:scale-95">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, i) => (
            <motion.div 
              ...
              className="group relative h-[450px] rounded-[2.5rem] overflow-hidden border border-border/70 glass shadow-sm hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500"
            >
              <div className={`absolute top-0 left-0 w-full h-2 bg-gradient-to-r ${cat.color} opacity-0 group-hover:opacity-100 transition-opacity`} />
              ...
              <div className="p-10 h-full flex flex-col justify-between relative z-10">
                <div className="space-y-6">
                   ...
                   <h3 className="text-2xl font-black text-foreground group-hover:text-primary transition-colors font-editorial tracking-tight">{cat.title}</h3>
                   <p className="text-secondary font-medium text-sm leading-relaxed line-clamp-4">
                     {cat.desc}
                   </p>
                </div>
                ...
                <div className="space-y-4 pt-6 border-t border-border">
                   <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-black text-foreground">{cat.stats}</span>
                   </div>
                   <Link href={`/get-started?category=${cat.title.toLowerCase()}`} className="w-full py-3 rounded-lg bg-muted/50 border border-border text-xs font-bold text-center group-hover:bg-primary group-hover:text-primary-foreground transition-all">
                      Select Category
                   </Link>
                </div>
              </div>
